					...
					if (iBalance <= iAmount) {
						...
						if ((Amount.isEmpty() || RecID.isEmpty())) {
							...
							if (flag != 0) {
								...
								String gRecBalance = "SELECT * FROM bankaccount WHERE bankaccount.BankAccountID = '"
								...
								while (resBalance.next()) {
									if (resBalance.getString("BankAccountID").toString()
											.equals(String.valueOf(RecID))) {
								...
								String updateRecQBalance = "UPDATE bankaccount SET BACurrentBalance = " + "'"
										+ (iRecBalance + iAmount) + "'" + "WHERE (BankAccountID = '"
										+ Integer.valueOf(RecID) + "');";
