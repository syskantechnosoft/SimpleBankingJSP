...
try {
	...
	String url = "jdbc:mysql://localhost:3306/syskan";
	String userDB = "root";
...
<body>
	...
	<h3 style="color: #5144F3">
		Your balance:
		<%=balance%></h3>

	<form action="transfer" method="Post">
		<input type="text" class="form-control" name="toid"
			placeholder="Account ID"> <input type="text"
			class="form-control" name="amount" placeholder="Amount"> <input
			type="hidden" name="myhiddenvalueID" value=<%=BankAccount%> /> <input
			type="hidden" name="myhiddenvalueBalance" value=<%=balance%> /> <input
			type="submit" class="createTransaction" value="Create Transaction">
	</form>
	<table border="1">
		<tr>
		...
		<tr>
			...
			<td>
				...
				%>
				<form>
					<input type="submit" class="cancelTransactionDisabled"
						value="Cancel Transaction" disabled>
				</form> <%} else {%>
				<form action="cancelTransaction" method="Post">
					...
					<input type="hidden" name="toid"
						value=<%=tResultSet.getString("BTToAccountID")%> /> <input
						type="hidden" name="amountC"
						value=<%=tResultSet.getString("BTAmount")%> /> <input
						type="hidden" name="senderBalance" value=<%=balance%> /> <input
						type="hidden" name="transactionID"
						value=<%=tResultSet.getString("BankTransactionID")%> /> <input
						type="submit" class="cancelTransaction" value="Cancel Transaction">
				</form> <%}%>
			</td>
		</tr>
		<%}%>
