...
<body>
	<div class="container">
		<form class="txtform">
			...
			%>
			<h3
				style="text-align: center; margin: 50px; font-family: Roboto; color: #27a327">
				Bank Account ID:
				<%=BankAccount%></h3>
			<%
			} else {
			%>
			<h3
				style="text-align: center; margin: 50px; font-family: Roboto; color: red">You
				don’t have a bank account.</h3>
			<%}%>
		...
		<form action="addaccount" method="Get">
			...
			if (!BankAccount.isEmpty()) {
			%>
			<input type="submit" class="btnAddAcountDisabled" value="Add Account"
				disabled>
			<%
			} else {
			%>
		...
		<form action="transactions.jsp" method="Post">
			...
			%>
			<input type="submit" class="btnTransactionDisabled"
				value="Transactions" disabled>
			<%}%>
		</form>
